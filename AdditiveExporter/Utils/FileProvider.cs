    ...
    public class FileProvider
    {
        ...
        public static async Task Init()
        {
            try
            {
                ...
                string aesResponse = await HttpClient.GetStringAsync($"https://uedb.dev/svc/api/v1/fortnite/aes{aesOverride}"); // this should probably be replaced with uedb soon
                ...
                var oodlePath = Path.Combine(Constants.DataPath, OodleHelper.OODLE_NAME_CURRENT);
                if (File.Exists(OodleHelper.OODLE_NAME_CURRENT))
                {
                    File.Move(OodleHelper.OODLE_NAME_CURRENT, oodlePath, true);
                }
